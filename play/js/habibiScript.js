...
function setPlayerAddress(accounts)
{
  playerAddress = accounts.length > 0 ? accounts[0] : null;
  if (playerAddress) {
    $("#walletStatus").text("Player : " + shortAddress(playerAddress));
...
// Au chargement : le joueur a-t-il deja autorise le site ? (sans popup)
async function initWallet()
{
  ...
  setPlayerAddress(accounts);
  window.ethereum.on("accountsChanged", setPlayerAddress);
}
...
/*****************************************************************************************************************************/
/*  Mint du score (MNS Coin sur Polygon Amoy)  ******************************************************************************/
/*****************************************************************************************************************************/
// ABI minimale : seule la fonction mint du contrat est appelee
const MINT_ABI = [{
...
// Le bundle UMD de web3 v4 expose selon les versions Web3 ou Web3.Web3.
function getWeb3Constructor()
{
...
async function mintScore(score)
{
  ...
  try {
    ...
    const web3 = new Web3Ctor(window.ethereum);
    const token = new web3.eth.Contract(MINT_ABI, TOKEN_ADDRESS);

    // Le token a 18 decimales : un score de 70 donne 70 MNS
    const amount = Web3Ctor.utils.toWei(String(score), "ether");

    const receipt = await token.methods.mint(playerAddress, amount)
      .send({
        from: playerAddress,
        maxPriorityFeePerGas: Web3Ctor.utils.toWei(AMOY_PRIORITY_FEE_GWEI, "gwei"),
        maxFeePerGas: Web3Ctor.utils.toWei(AMOY_MAX_FEE_GWEI, "gwei")
      })
      .on("transactionHash", function(hash) {
        $("#mintStatus").text("Waiting for confirmation... tx " + hash.slice(0, 10) + "...");
      });
...
$("#sbmt-score").click(async function(){
  ...
  await mintScore(gameEngine.score);
});
...
function showHighscoreMessage(message)
{
  $("#highscoreList").empty().append($("<li>").text(message));
}

async function GetTokenTransactions()
{
  showHighscoreMessage("Loading token transactions...");
  try {
    const Web3Ctor = getWeb3Constructor();
    const web3 = new Web3Ctor(AMOY_READ_RPC);
    const token = new web3.eth.Contract(TRANSFER_ABI, TOKEN_ADDRESS);

    // Un mint est un Transfer depuis l'adresse zero : on lit ceux faits
    // par le jeu, apres le bloc du deploiement, par tranches de 10 000 blocs
    const latest = Number(await web3.eth.getBlockNumber());
    let events = [];
    for (let from = TOKEN_DEPLOY_BLOCK + 1; from <= latest; from += LOGS_BLOCK_RANGE) {
      const to = Math.min(from + LOGS_BLOCK_RANGE - 1, latest);
      const chunk = await token.getPastEvents("Transfer", {
    ...
    if (events.length === 0) {
      showHighscoreMessage("No token transactions yet");
      return;
    }

    // La date n'est pas dans l'evenement : on lit l'heure de chaque bloc
    const timestamps = {};
    for (const ev of events) {
      const blockNumber = String(ev.blockNumber);
    ...
    $("#highscoreList").empty();
    events.forEach(ev => {
      const date = new Date(timestamps[String(ev.blockNumber)] * 1000);
      const amount = Web3Ctor.utils.fromWei(ev.returnValues.value, "ether");

      // Valeurs inserees en texte (et non en HTML) pour eviter toute injection
      const $li = $("<li>").addClass("txitem");
      $li.append($('<p class="firstitem">').text(date.toLocaleString()));
      $li.append($('<p class="seconditem">').text(amount + " MNS"));
      $li.append($('<p class="thirditem">').text(ev.transactionHash.slice(0, 6)));
      $("#highscoreList").append($li);
    });
  } catch (e) {
    console.error("Error loading token transactions", e);
    showHighscoreMessage("Unable to load token transactions");
  }
}
